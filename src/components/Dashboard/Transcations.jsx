...
const CustomCheckbox = ({ label, checked, onChange }) => (
  ...
  >
    ...
    <div className={`w-4 h-4 rounded-[4px] border flex items-center justify-center transition-colors duration-200
      ${checked ? 'bg-[#8a3c46] border-[#8a3c46]' : 'border-gray-300'}`}>
      {checked && (
...
const DashboardLayout = () => {
  ...
  const handleClearAll = () => {
    setTypeFilters({
      'All': false, 'Recharge & Bills': false, 'Referral Bonus': false, 'Zonal Royalty': false
    });
    setStatusFilters({
      'All': false, 'Success': false, 'Pending': false, 'Failed': false
    });
  };
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex flex-col overflow-hidden w-[384px] h-[464px] shrink-0">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  className="text-sm font-medium text-[#8a3c46] hover:underline"
                >
                  Clear All
                </button>
              </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-3 cursor-pointer">
                    <h3 className="font-semibold text-gray-800 text-sm">
                      Transaction Type <span className="text-[#8a3c46]">(02)</span>
                    </h3>
              ...
              <div className="flex justify-between items-center mb-6 ">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm w-72 focus:outline-none focus:border-gray-400"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-0">
                {listTransactions.map((tx, index) => (
                  <div key={tx.id}>
                    <div className="flex items-center justify-between py-5">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-[#fcf3f3] flex items-center justify-center shrink-0">
                          {tx.iconType === 'out' ? (
                            <ArrowUpRight className="w-6 h-6 text-[#8a3c46]" />
                          ) : (
                            <ArrowDownLeft className="w-6 h-6 text-[#8a3c46]" />
                          )}
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        <p className={`font-bold text-[15px] mb-1 ${tx.iconType === 'in' ? 'text-green-600' : 'text-gray-900'}`}>
                          {tx.amount}
                        </p>
                        <p className={`text-xs font-medium ${tx.status === 'Success' ? 'text-green-600' : 'text-red-500'}`}>
                          {tx.status}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Separator Line */}
                    {index !== listTransactions.length - 1 && (
